...
<li><a class="app-menu__item" href="kelola_suratkeluarkd.php"><i class="app-menu__icon fa fa-paper-plane"></i>
  ...
  <main class="app-content">
    <div class="app-title">
      <div>
        <h1><i class="fa fa-book"></i> Grafik Surat Masuk</h1>
   ...
   <?php
    include 'koneksi.php';
    $a= mysqli_query($db, "SELECT * from surat_masuk where disposisi_tujuan='Audit Internal'");
    $jml1= $a->num_rows;
    $b= mysqli_query($db, "SELECT * from surat_masuk where disposisi_tujuan='Sekretaris Perusahaan'");
    $jml2= $b->num_rows;
    $c= mysqli_query($db, "SELECT * from surat_masuk where disposisi_tujuan='Perencanaan & Pengembangan Bisnis'");
    $jml3= $c->num_rows;
    $d= mysqli_query($db, "SELECT * from surat_masuk where disposisi_tujuan='Teknologi Informasi'");
    $jml4= $d->num_rows;
    $e= mysqli_query($db, "SELECT * from surat_masuk where disposisi_tujuan='Tanaman'");
    $jml5= $e->num_rows;
    $f= mysqli_query($db, "SELECT * from surat_masuk where disposisi_tujuan='Akutansi'");
    $jml6= $f->num_rows;
    $g= mysqli_query($db, "SELECT * from surat_masuk where disposisi_tujuan='Keuangan'");
    $jml7= $g->num_rows;
    $h= mysqli_query($db, "SELECT * from surat_masuk where disposisi_tujuan='Pengadaan Barang & Jasa'");
    $jml8= $h->num_rows;
    $i= mysqli_query($db, "SELECT * from surat_masuk where disposisi_tujuan='Modal Manusia'");
    $jml9= $i->num_rows;
    $j= mysqli_query($db, "SELECT * from surat_masuk where disposisi_tujuan='Hukum & Umum'");
    $jml10= $j->num_rows;
    $k= mysqli_query($db, "SELECT * from surat_masuk where disposisi_tujuan='Pemasaran & QA'");
    $jml11= $k->num_rows;
    $l= mysqli_query($db, "SELECT * from surat_masuk where disposisi_tujuan='Teknik & Pengolahan'");
    $jml12= $l->num_rows;
   ?>
